...
面向 2026 级新生的移动端优先网页。参与者完成 25 道情境选择后，会得到四维科创倾向、1 个首选部门和 2 个备选部门，并可查看七个部门介绍和招新群入口。
...
调整题库时需保留 25 题结构：17 道倾向题、8 道工作场景题。选项中不直接写部门名称，场景计分机会保持均衡；修改后至少运行 `pnpm test` 和 `pnpm test:e2e`。
